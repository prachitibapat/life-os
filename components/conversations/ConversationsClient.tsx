...
export function ConversationsClient() {
  ...
  const save = async () => {
    const res = await fetch('/api/conversations', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ ...form, growth_areas: form.growth_areas.split(',').map(s=>s.trim()).filter(Boolean) }) });
    if (res.ok) { const d = await res.json(); setLogs([d,...logs]); setOpen(false); toast.success('Logged'); }
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><Label>Context</Label><Input value={form.context} onChange={e=>setForm({...form,context:e.target.value})} placeholder="Team meeting, 1:1, etc." /></div>
              </div>
